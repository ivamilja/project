...
public class OnlineOrderResource {
    ...
    public OnlineOrderResource(OnlineOrderRepository onlineOrderRepository, OnlineOrderSearchRepository onlineOrderSearchRepository) {
        this.onlineOrderRepository = onlineOrderRepository;
        this.onlineOrderSearchRepository = onlineOrderSearchRepository;
    }
    ...
    public ResponseEntity<OnlineOrder> createOnlineOrder(@Valid @RequestBody OnlineOrder onlineOrder) throws URISyntaxException {
        ...
        onlineOrderSearchRepository.save(result);
    ...
    public ResponseEntity<Void> deleteOnlineOrder(@PathVariable Long id) {
        ...
        onlineOrderSearchRepository.deleteById(id);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, id.toString())).build();
    }

    /**
     * SEARCH  /_search/online-orders?query=:query : search for the onlineOrder corresponding
     * to the query.
     *
     * @param query the query of the onlineOrder search
     * @return the result of the search
     */
    @GetMapping("/_search/online-orders")
    @Timed
    public List<OnlineOrder> searchOnlineOrders(@RequestParam String query) {
        log.debug("REST request to search OnlineOrders for query {}", query);
        return StreamSupport
            .stream(onlineOrderSearchRepository.search(queryStringQuery(query)).spliterator(), false)
