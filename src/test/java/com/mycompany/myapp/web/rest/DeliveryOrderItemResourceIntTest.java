...
public class DeliveryOrderItemResourceIntTest {
     ...
     *
     * @see com.mycompany.myapp.repository.search.DeliveryOrderItemSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void setup() {
        ...
        final DeliveryOrderItemResource deliveryOrderItemResource = new DeliveryOrderItemResource(deliveryOrderItemRepository, mockDeliveryOrderItemSearchRepository);
    ...
    public void createDeliveryOrderItem() throws Exception {
        ...
        // Validate the DeliveryOrderItem in Elasticsearch
        verify(mockDeliveryOrderItemSearchRepository, times(1)).save(testDeliveryOrderItem);
    }
    ...
    public void createDeliveryOrderItemWithExistingId() throws Exception {
        ...
        // Validate the DeliveryOrderItem in Elasticsearch
        verify(mockDeliveryOrderItemSearchRepository, times(0)).save(deliveryOrderItem);
    }
    ...
    public void deleteDeliveryOrderItem() throws Exception {
        ...
        // Validate the DeliveryOrderItem in Elasticsearch
        verify(mockDeliveryOrderItemSearchRepository, times(1)).deleteById(deliveryOrderItem.getId());
    }

    @Test
    @Transactional
    public void searchDeliveryOrderItem() throws Exception {
        // Initialize the database
        deliveryOrderItemRepository.saveAndFlush(deliveryOrderItem);
        when(mockDeliveryOrderItemSearchRepository.search(queryStringQuery("id:" + deliveryOrderItem.getId())))
            .thenReturn(Collections.singletonList(deliveryOrderItem));
        // Search the deliveryOrderItem
        restDeliveryOrderItemMockMvc.perform(get("/api/_search/delivery-order-items?query=id:" + deliveryOrderItem.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(deliveryOrderItem.getId().intValue())))
            .andExpect(jsonPath("$.[*].preparedAmount").value(hasItem(DEFAULT_PREPARED_AMOUNT.intValue())))
