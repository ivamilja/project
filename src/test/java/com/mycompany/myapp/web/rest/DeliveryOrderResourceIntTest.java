...
public class DeliveryOrderResourceIntTest {
     ...
     *
     * @see com.mycompany.myapp.repository.search.DeliveryOrderSearchRepositoryMockConfiguration
     */
    @Autowired
    private DeliveryOrderSearchRepository mockDeliveryOrderSearchRepository;
    ...
    public void setup() {
        ...
        final DeliveryOrderResource deliveryOrderResource = new DeliveryOrderResource(deliveryOrderRepository, mockDeliveryOrderSearchRepository);
    ...
    public void createDeliveryOrder() throws Exception {
        ...
        // Validate the DeliveryOrder in Elasticsearch
        verify(mockDeliveryOrderSearchRepository, times(1)).save(testDeliveryOrder);
    }
    ...
    public void createDeliveryOrderWithExistingId() throws Exception {
        ...
        // Validate the DeliveryOrder in Elasticsearch
        verify(mockDeliveryOrderSearchRepository, times(0)).save(deliveryOrder);
    }
    ...
    public void deleteDeliveryOrder() throws Exception {
        ...
        // Validate the DeliveryOrder in Elasticsearch
        verify(mockDeliveryOrderSearchRepository, times(1)).deleteById(deliveryOrder.getId());
    }

    @Test
    @Transactional
    public void searchDeliveryOrder() throws Exception {
        // Initialize the database
        deliveryOrderRepository.saveAndFlush(deliveryOrder);
        when(mockDeliveryOrderSearchRepository.search(queryStringQuery("id:" + deliveryOrder.getId())))
            .thenReturn(Collections.singletonList(deliveryOrder));
        // Search the deliveryOrder
        restDeliveryOrderMockMvc.perform(get("/api/_search/delivery-orders?query=id:" + deliveryOrder.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(deliveryOrder.getId().intValue())))
            .andExpect(jsonPath("$.[*].deliveryDate").value(hasItem(DEFAULT_DELIVERY_DATE.toString())))
