...
public class OnlineOrderItemResourceIntTest {
     ...
     *
     * @see com.mycompany.myapp.repository.search.OnlineOrderItemSearchRepositoryMockConfiguration
     */
    @Autowired
    private OnlineOrderItemSearchRepository mockOnlineOrderItemSearchRepository;
    ...
    public void setup() {
        ...
        final OnlineOrderItemResource onlineOrderItemResource = new OnlineOrderItemResource(onlineOrderItemRepository, mockOnlineOrderItemSearchRepository);
    ...
    public void createOnlineOrderItem() throws Exception {
        ...
        // Validate the OnlineOrderItem in Elasticsearch
        verify(mockOnlineOrderItemSearchRepository, times(1)).save(testOnlineOrderItem);
    }
    ...
    public void createOnlineOrderItemWithExistingId() throws Exception {
        ...
        // Validate the OnlineOrderItem in Elasticsearch
        verify(mockOnlineOrderItemSearchRepository, times(0)).save(onlineOrderItem);
    }
    ...
    public void deleteOnlineOrderItem() throws Exception {
        ...
        // Validate the OnlineOrderItem in Elasticsearch
        verify(mockOnlineOrderItemSearchRepository, times(1)).deleteById(onlineOrderItem.getId());
    }

    @Test
    @Transactional
    public void searchOnlineOrderItem() throws Exception {
        // Initialize the database
        onlineOrderItemRepository.saveAndFlush(onlineOrderItem);
        when(mockOnlineOrderItemSearchRepository.search(queryStringQuery("id:" + onlineOrderItem.getId())))
            .thenReturn(Collections.singletonList(onlineOrderItem));
        // Search the onlineOrderItem
        restOnlineOrderItemMockMvc.perform(get("/api/_search/online-order-items?query=id:" + onlineOrderItem.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(onlineOrderItem.getId().intValue())))
            .andExpect(jsonPath("$.[*].orderedAmount").value(hasItem(DEFAULT_ORDERED_AMOUNT.intValue())))
