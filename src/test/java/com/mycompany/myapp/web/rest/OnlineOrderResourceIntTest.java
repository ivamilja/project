...
import com.mycompany.myapp.NoviApp;

import com.mycompany.myapp.domain.OnlineOrder;
import com.mycompany.myapp.domain.City;
import com.mycompany.myapp.domain.Client;
import com.mycompany.myapp.repository.OnlineOrderRepository;
import com.mycompany.myapp.repository.search.OnlineOrderSearchRepository;
...
@SpringBootTest(classes = NoviApp.class)
public class OnlineOrderResourceIntTest {
     ...
     *
     * @see com.mycompany.myapp.repository.search.OnlineOrderSearchRepositoryMockConfiguration
     */
    @Autowired
    private OnlineOrderSearchRepository mockOnlineOrderSearchRepository;
    ...
    public void setup() {
        ...
        final OnlineOrderResource onlineOrderResource = new OnlineOrderResource(onlineOrderRepository, mockOnlineOrderSearchRepository);
    ...
    public void createOnlineOrder() throws Exception {
        ...
        // Validate the OnlineOrder in Elasticsearch
        verify(mockOnlineOrderSearchRepository, times(1)).save(testOnlineOrder);
    }
    ...
    public void createOnlineOrderWithExistingId() throws Exception {
        ...
        // Validate the OnlineOrder in Elasticsearch
        verify(mockOnlineOrderSearchRepository, times(0)).save(onlineOrder);
    }
    ...
    public void deleteOnlineOrder() throws Exception {
        ...
        // Validate the OnlineOrder in Elasticsearch
        verify(mockOnlineOrderSearchRepository, times(1)).deleteById(onlineOrder.getId());
    }

    @Test
    @Transactional
    public void searchOnlineOrder() throws Exception {
        // Initialize the database
        onlineOrderRepository.saveAndFlush(onlineOrder);
        when(mockOnlineOrderSearchRepository.search(queryStringQuery("id:" + onlineOrder.getId())))
            .thenReturn(Collections.singletonList(onlineOrder));
        // Search the onlineOrder
        restOnlineOrderMockMvc.perform(get("/api/_search/online-orders?query=id:" + onlineOrder.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(onlineOrder.getId().intValue())))
            .andExpect(jsonPath("$.[*].address").value(hasItem(DEFAULT_ADDRESS.toString())))
